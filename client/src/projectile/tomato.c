...
TomatoProjectile *tomatoprojectile_create(SDL_Point direction,
                                          SDL_Point coord) {
  TomatoProjectile *tomato = malloc(sizeof(TomatoProjectile));
  SDL_Point size = (SDL_Point){.x = 32, .y = 32};
  tomato->projectile =
      projectile_create(5.0f, 10.0f, size, tomato, PROJECTILE_TOMATO, direction,
                        (void (*)(void *))tomatoprojectile_destroy);

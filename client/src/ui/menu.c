...
Menu *menu_create(SDL_Renderer *renderer) {
  ...
  menu->background_texture =
      load_texture("assets/images/tomario.png", renderer);
  ...
  SDL_Rect play_rect = {290, 300, 200, 50};
  Button *play_button = button_create(renderer, "Play Game", font, normal_color,
                                      hover_color, play_rect, NULL, NULL);
  menu->play_button_element =
      element_create(play_rect, (void (*)(void *))button_draw, play_button);

  SDL_Rect exit_rect = {290, 400, 200, 50};
  Button *exit_button = button_create(renderer, "Exit Game", font, normal_color,
                                      hover_color, exit_rect, NULL, NULL);
  menu->exit_button_element =
      element_create(exit_rect, (void (*)(void *))button_draw, exit_button);
  ...
  Text *ip_text = text_create(renderer, "IP Address: 127.0.0.1",
                              "assets/fonts/sans.ttf", 24);
  text_set_color(ip_text, text_color);
  text_set_coordinate(ip_text, (SDL_Point){300, 175});
  menu->ip_element = text_get_element(ip_text);

  Text *port_text =
      text_create(renderer, "Port: 3030", "assets/fonts/sans.ttf", 24);
  text_set_color(port_text, text_color);
  text_set_coordinate(port_text, (SDL_Point){300, 225});
...
void update_text_element(Element *element, const char *new_text,
...
void menu_render(Menu *menu, SDL_Renderer *renderer) {
  ...
  SDL_Rect ip_bar_rect = {190, 170, 400, 40};
  SDL_RenderFillRect(renderer, &ip_bar_rect);
  SDL_Rect port_bar_rect = {190, 220, 400, 40};
...
int menu_handle_event(Menu *menu, SDL_Event *event) {
  Button *play_button =
      (Button *)element_get_coomponent(menu->play_button_element);
  Button *exit_button =
      (Button *)element_get_coomponent(menu->exit_button_element);
  ...
  if (event->type == SDL_MOUSEBUTTONDOWN) {
    ...
    if (x >= ip_rect.x && x <= ip_rect.x + ip_rect.w && y >= ip_rect.y &&
        y <= ip_rect.y + ip_rect.h) {
      menu->active = 1;
      return 0;
    }
    ...
    if (menu->active == 1 &&
        strlen(menu->ip_address) < 15) // 15(?) 198.168.0.1 [xxxxxxxxxxxx]?
